...
\end{equation}
donde H  es el operador Hamiltoniano del sistema. La entropía que se va a maximizar  será la entropía de Gibbs, de esta se hablará un poco más profundo en el siguiente capítulo, esta se escribe para un sistema cuántico
\begin{equation}
...
\begin{equation}
\rho= \exp [ (\frac{ \alpha }{k_{B}}-1 ) I + \frac{\beta}{k_{B}}H ].
\end{equation}
...
\end{equation}
Aquí se ha definido la función de partición $Z(\beta)$ como la normalización del operador de densidad de probabilidad. Por medio de las restricciones se puede encontrar que el multiplicador $\beta$ es $\beta= \frac{1}{T}$. Entonces se puede reescribir el operador de densidad de probabilidad como
\begin{equation}
...
\begin{theorem}
Sea $\{ E_{m}\}$ un POVM, con $p_{m} \equiv \Tr(\rho E_{m})$ y $q_{m}\equiv \Tr(\sigma E_{m})$ como las probabilidades de obtener un el resultado de una medida $m$. Luego
...
\begin{equation}
D(p_{m},q_{m})= \frac{1}{2} \sum_{m} |\Tr(E_{m}(\rho- \sigma))|.
\end{equation}
...
\quad &= D(\rho,\sigma),
\end{align}
se usó la relación de completitud de los elementos de POVM. Por otro lado si se escogen medidas donde los elementos POVM incluyen proyectores sobre e soporte de $Q$ y $S$, se ve que existen mediciones que dan distribuciones de probabilidad tales que $D(p_{m},q_{m})= D(\rho,\sigma)$.
